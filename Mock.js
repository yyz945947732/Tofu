...
class Mock {
  ...
  __arrayMockAction(props, num) {
    let i = 0;
    this.__defineProperty('length', num);
    while (num > 0) {
      this[i] = this.__propsController(props, {});
      i++;
      num--;
    }
  ...
  __propsController(props, host = this) {
    for (const i in props) {
      ...
      const value = this.__getValueByType(type, props[i])
      if (value !== false) host[i] = value
    }
    return host
  }
  __getValueByType(type, value) {
    switch (type) {
      case 'number':
        return this.__numberAction(value);
      case 'string':
        return this.__stringAction(value);
      case 'object':
        return this.__objectAction(value);
      default:
        return false
    }
  }
  ...
  __objectAction(value) {
    if (this.__isObject(value)) {
      return this.__propsController(value, value)
    } else if (value instanceof Array) {
      return this.__random(value)
    } else {
      return false
    }
  }
  __stringAction(type) {
    ...
    switch (strType) {
      case 'address':
        return this.__getRandomWords(2) + '省 ' + this.__getRandomWords(2) + '市 ' + this.__getRandomWords(2) + '区 ' + this.__getRandomWords(3) + '街道';
      case 'img':
        return this.__random(img);
      case 'id':
        return this.__getId();
      default:
        return this.__numberAction(type.length);
    }
  }
  __getId() {
    return this.__id++;
  ...
  __numberAction(num) {
    return this.__getRandomWords(this.__getLegalNum(num))
  }
  __random(arr) {
    return arr instanceof Array ? arr[Math.floor(Math.random() * arr.length)] : null
  }
  __getRandomWords(num) {
    const start = '%u';
    ...
    const getARandom = () => start + this.__random(words.split(','));
    function reCall(num, fn) {
      let answer = '';
      return function() {
        let i = num;
        while (i > 0) {
